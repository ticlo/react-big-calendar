const getKey = ({ min, max, step, slots, localizer }) => `${+localizer.startOf(min, 'minutes')}` +
    `${+localizer.startOf(max, 'minutes')}` +
    `${step}-${slots}`;
export function getSlotMetrics({ min: start, max: end, step, timeslots, localizer, }) {
    ...
    function positionFromDate(date) {
        const diff = localizer.diff(start, date, 'minutes') +
    ...
    return {
        ...
        nextSlot(slot) {
            ...
            let next = slots[Math.min(slots.findIndex((s) => s === slot || localizer.eq(s, slot)) + 1, slots.length - 1)];
        ...
        closestSlotFromDate(date, offset = 0) {
            if (localizer.lt(date, start, 'minutes'))
                return slots[0];
