...
class DayColumn extends React.Component {
    ...
    render() {
        const { date, max, rtl, isNow, resource, accessors, localizer, getters: { dayProp, ...getters }, components: { eventContainerWrapper: EventContainer, ...components }, } = this.props;
        ...
        return (React.createElement(DayColumnWrapperComponent, { ref: this.containerRef, date: date, style: style, className: clsx(className, 'rbc-day-slot', 'rbc-time-column', isNow && 'rbc-now', isNow && 'rbc-today', // WHY
            selecting && 'rbc-slot-selecting'), slotMetrics: slotMetrics },
            slotMetrics.groups.map((grp, idx) => (React.createElement(TimeSlotGroup, { key: idx, group: grp, resource: resource, getters: getters, components: components }))),
            React.createElement(EventContainer, { localizer: localizer, resource: resource, accessors: accessors, getters: getters, components: components, slotMetrics: slotMetrics },
                React.createElement("div", { className: clsx('rbc-events-container', rtl && 'rtl') },
                    this.renderEvents({
                        events: this.props.backgroundEvents,
                        isBackgroundEvent: true,
                    }),
                    this.renderEvents({ events: this.props.events }))),
            selecting && (React.createElement("div", { className: "rbc-slot-selection", style: { top, height } },
                React.createElement("span", null, localizer.format(selectDates, 'selectRangeFormat')))),
            isNow && this.intervalTriggered && (React.createElement("div", { className: "rbc-current-time-indicator", style: { top: `${this.state.timeIndicatorPosition}%` } }))));
    }
    renderEvents = ({ events, isBackgroundEvent }) => {
        let { rtl, selected, accessors, localizer, getters, components, step, timeslots, dayLayoutAlgorithm, resizable, } = this.props;
        ...
        return styledEvents.map(({ event, style }, idx) => {
            ...
            return (React.createElement(TimeGridEvent, { style: style, event: event, label: label, key: 'evt_' + idx, getters: getters, rtl: rtl, components: components, continuesPrior: continuesPrior, continuesAfter: continuesAfter, accessors: accessors, resource: this.props.resource, selected: isSelected(event, selected), onClick: (e) => this._select({
                    ...event,
                    ...(this.props.resource && {
                        sourceResource: this.props.resource,
                    }),
                    ...(isBackgroundEvent && { isBackgroundEvent: true }),
                }, e), onDoubleClick: (e) => this._doubleClick(event, e), isBackgroundEvent: isBackgroundEvent, onKeyPress: (e) => this._keyPress(event, e), resizable: resizable }));
        });
    };
    _selectable = () => {
        ...
        let selectionState = (point) => {
            ...
            const selectRange = this.slotMetrics.getRange(localizer.min(initialSlot, currentSlot), localizer.max(initialSlot, currentSlot));
            return {
                ...selectRange,
                selecting: true,
                top: `${selectRange.top}%`,
