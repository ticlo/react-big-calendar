...
WorkWeek.range = function (date, options) {
  ...
  var r = _Week.default.range(date, options).filter(function (d) {
    var weekday = localizer.add(d, 0, 'minutes').weekday;
...
WorkWeek.title = function (date, _ref) {
  ...
  var _WorkWeek$range = _WorkWeek.range(date, {
      localizer: localizer
    }),
    _WorkWeek$range2 = (0, _toArray2.default)(_WorkWeek$range),
    start = _WorkWeek$range2[0],
    rest = _WorkWeek$range2.slice(1);
